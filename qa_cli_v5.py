...
class QuestionAnswerCLI:
    ...
    def generate_question(self, context: str, sentiment: float) -> str:
        ...
        outputs = self.model.generate(
            input_ids,
            max_length=64,
            num_return_sequences=5,  # Reduced from 10 to 5
    ...
    def generate_rule_based_question(self, context: str, sentiment: float) -> str:
        ...
        general_questions = [
            f"{self.user_name}, what are your thoughts on {random.choice(words)}?",
            f"How does {random.choice(words)} relate to the overall topic, {self.user_name}?",
            f"{self.user_name}, can you elaborate on the concept of {random.choice(words)}?",
